...
export default async (req, res) => {
    ...
    let array = await db.collection('charitylocations').get()
    .then((querySnapshot) => {
        let docsarr = new Array(querySnapshot.size);
        let i = 0;
        console.log("got " + docsarr.length + " geopoints");
        querySnapshot.forEach((doc) => {
            console.log(doc.id, " => ", doc.get("l") + " are coords of |||" + doc.get("name"));
            if (doc.get("l") == null) 
            {
              return;
            }
            ...
            { 
                ...
                "geometry": { 
                    "type": "Point", 
                    "coordinates": doc.get("l").reverse() //TODO check recieved doc is valid - death otherwise
                }
            };
            console.log(charitylocation);
            docsarr[i] = charitylocation;
            i += 1;
            console.log(charitylocation + " end");
        });
        return docsarr;
    })
    res.statusCode = 200;
    console.log("returning: " + array.length + "|" + array);
    res.json({
      type: "FeatureCollection",
      features: array 
    });
  };
/* the returing geojson structure
{
    "type":"FeatureCollection",
    "features":
    [
        {
            ...
            {
                "type":"Point",
                "coordinates":[55.81332708539361,36.491916328668594]
            },
            "properties":
            {
                "title":"alltags",
